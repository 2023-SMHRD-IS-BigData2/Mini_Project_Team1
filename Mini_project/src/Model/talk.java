...
public class talk {
	...
	public void Synopsis(String id) {
		System.out.println();
		try {
			Thread.sleep(50);
	...
	public void Scenario(String id) {
		...
		System.out.println(id + "씨 : 하하 네 그렇습니다.(부장님이 프로젝트 반려만 안하시면 말이죠)");
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		System.out.println("회사 사람들 : 헉... " + id + "씨 어떡해 어떡해");
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		System.out.println("마부장 : 걱정마 걱정마 내가 뭐 나쁜짓하는것도 아니고 겨우 쌈싸주긴데 뭐 어때 그러니 " + id + "씨 쌈 한번 야무지게 싸봐");
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		System.out.println(id + "씨 : 제가요?");
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		System.out.println("마부장 : 제가요? 하하하하 우리 " + id + "아주 mz해 하하하 그런 " + id + "씨는 나 먹을 쌈도 아주 mz하게 싸주겠지"
		+ "나 기대할께");
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
	...
	public void SucScenario(String id) {
		System.out.println("마부장 : 오 우리 " + id + "씨 쌈 잘싸는데 내 취향에 완벽 부합한 다시 없을 쌈이었어");
		System.out.println(id + "씨 정말 맘에 드는구만 그래 하하하하");
		System.out.println(id + "씨 : 이정도는 기본이죠 하하하(휴~~~~~)");
		
	}
	
	public void FailScenario(String id) {
		System.out.println("마부장 : 에잇 퉤 " + id + "씨 이걸 지금 쌈이라고 싼거야? " + id + "씨 정말 안되겠네 이거");
		System.out.println("회사 사람들 : 아이고 또 시작이네");
		System.out.println(id + "씨 : (그럼 부장님이 직접 싸시든가요!!!!!!!!!!!) ");
	...
	public void EasySucScenario(String id) {
		System.out.println("마부장 : 하하 " + id + "씨 맛있는데 하하 쌈 잘싸는데 " + id + "씨");
		System.out.println(id + "씨 : 하하 뭘 이정도 가지고 제가 한 쌈 할줄 압니다. 하하하");
	}
	
	public void EasyFailScenario(String id) {
		System.out.println("마부장 : " + id + "씨.... 맛있네.. 그치만 어디 가서 쌈 싸준다고 말하고 다니지 않았으면 좋겠어");
		System.out.println(id + "씨 : .....");
	}
	...
	public void NormalSucScenario(String id) {
		System.out.println("마부장 : 하하하 " + id + "씨 맛있어 자네 아주 그냥 쌈의 달인이구만 그 프로젝트는 천천히 기다리지 크하하하");
		System.out.println(id + "씨 : 감사합니다. 마부장님");
	}
	
	public void NormalFailScenario(String id) {
		System.out.println("마부장 : 퉤퉤" + id + "씨 이걸 지금 사람 먹으라고 싼 쌈이야? 나랑 싸우자는 거지");
